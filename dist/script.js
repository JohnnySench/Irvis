...
const modals = () => {
  const bindModals = (selectorTrigger, selectorModal, selectorClose) => {
    ...
    const toggleModal = (display, overflow) => {
      // Ф-ция закрытия и открытия модального окна
      modal.style.display = display;
      document.body.style.overflow = overflow;
    };
    trigger.forEach(btn => {
      // Перебираются все кнопки и вешается обработчик, чтобы при нажатии открывалась модалка
    ...
    closeBtn.addEventListener('click', () => {
      // Модальное окно закрывается если на крестик
      toggleModal('none', '');
    });
    modal.addEventListener('click', e => {
      // Модальное окно закрывается если нажали на подложку
      if (e.target === modal) {
        toggleModal('none', '');
      }
    });
  };
  const showModalByTime = (selectorModal, time) => {
